...
document.addEventListener("DOMContentLoaded", () => {
	...
	function updateThemeIcon(theme) {
		...
		if (theme === "dark") {
			if (elements.themeIconLight)
				elements.themeIconLight.classList.remove("hidden");
			if (elements.themeIconDark)
	...
	function handleTranslate() {
		...
		const selectedModels = Array.from(
			elements.modelParams.querySelectorAll("input:checked"),
		).map((cb) => cb.value);

		if (!query) return showToast(t("toast_enter_text"), "error");
		if (selectedModels.length < 2)
			return showToast(t("toast_select_models"), "error");
	...
	function renderTranslation(data) {
		...
		if (data.response_hash) {
			if (seenHashes.has(data.response_hash)) {
				...
				badge.style.cssText =
					"background: #ff9800; color: #fff; padding: 2px 6px; border-radius: 4px; font-size: 0.8em; margin-left: 8px; vertical-align: middle;";
	...
	function setupEventListeners() {
		...
		document.addEventListener("click", (e) => {
			if (
				elements.userMenuDropdown &&
				!elements.userMenuDropdown.contains(e.target) &&
				e.target !== elements.userMenuBtn
			) {
		...
		if (elements.submitVotesBtn)
			elements.submitVotesBtn.addEventListener("click", submitVotes);
		if (elements.copyJsonBtn)
			elements.copyJsonBtn.addEventListener("click", copyJSON);
		if (elements.copyPromptBtn)
			elements.copyPromptBtn.addEventListener("click", copyAnalysisPrompt);
...
- 2 Stars (Good): Correct meaning, but needs polish/editing.
- 1 Star (Okay): Errors or mistakes present, but generally understandable.
- Reject (Trash): Didn't follow instructions, gibberish, or entirely wrong meaning.`;
