document.addEventListener("DOMContentLoaded", () => {
	...
	if (elements.clearFiltersBtn) {
		elements.clearFiltersBtn.addEventListener("click", () => {
			document.querySelectorAll(".model-filter-checkbox").forEach((cb) => {
				cb.checked = false;
			});
	...
	function renderModelFilters() {
		...
		const sortedKeys = Object.keys(availableModels).sort((a, b) => {
			return (availableModels[a].name || a).localeCompare(
				availableModels[b].name || b,
			);
		});
	...
	function updateSelectedModels() {
		...
		document
			.querySelectorAll(".model-filter-checkbox:checked")
			.forEach((cb) => {
		...
		if (selectedModels.size > 0) {
			...
			if (elements.activeFilterMsg) {
				...
				elements.activeFilterList.textContent = Array.from(selectedModels)
					.map((k) => availableModels[k]?.name || k)
					.join(", ");
			}
			...
			if (elements.filterCountBadge)
				elements.filterCountBadge.classList.add("hidden");
			if (elements.activeFilterMsg)
				elements.activeFilterMsg.classList.add("hidden");
	...
	function renderStats(stats) {
		...
		const remainingText =
			stats.pairs_remaining !== undefined
				? ` (${stats.pairs_remaining} ${t("stats_remaining")})`
				: "";
...
\`\`\``;
	...
	const compareCopyPromptBtn = document.getElementById(
		"compare-copy-prompt-btn",
	);
